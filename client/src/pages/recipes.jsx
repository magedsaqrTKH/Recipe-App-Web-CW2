...
const Recipes = () => {
  ...
  const saveRecipes = async () => {
    try {
      await axios.post('http://localhost:3001/recipes/save', {
        ...
        cookingTime: info.readyInMinutes,
        difficulty: info.difficulty,
        nutritionalInfo: {
          calories: info.nutrition?.nutrients.find(nutrient => nutrient.title === 'Calories')?.amount,
          protein: info.nutrition?.nutrients.find(nutrient => nutrient.title === 'Protein')?.amount,
          carbs: info.nutrition?.nutrients.find(nutrient => nutrient.title === 'Carbohydrates')?.amount,
          fats: info.nutrition?.nutrients.find(nutrient => nutrient.title === 'Fat')?.amount
        },
  ...
  return (
    ...
    <Button onClick={saveRecipes}>Save to Favorites</Button>
    </div>
    <Info>
    <Button
